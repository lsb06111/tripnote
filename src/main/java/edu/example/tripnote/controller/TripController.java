...
public class TripController {
	...
	public String goPlan(TripInfoDTO tripInfoDTO, Model model) {
		...
		List<AttractionDTO> list = getAttractions(tripInfoDTO.getTripDest());
		model.addAttribute("attList", list);
		System.out.println(list.size());
		return "trip/index2";
	}
	
	public List<AttractionDTO> getAttractions(String destinationName){
		...
		for (AttractionDTO dto : attractionList) {
			...
			try {
				...
				dto.setOverview("소개 정보를 불러오는 데 실패했습니다.");
				e.printStackTrace();
			}
		}
